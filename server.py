    ...
    while not transmissao_concluida:
        ...
        while DELIMITADOR in buffer_recebimento:
            ...
            if (num_pacote == num_sequencia_esperado) and (checksum_calculado == checksum_recebido):
                ...
                num_pacote_processado = num_pacote
                num_sequencia_esperado += 1

                ack_a_enviar = num_sequencia_esperado - 1

                if flag_fim == '1':
                    print(f"[SERVER] Flag de fim recebida. Enviando ACK final -> ACK_{ack_a_enviar}\n")
                    conexao_cliente.sendall(f"ACK_{ack_a_enviar}".encode('utf-8'))
                    print("[SERVER] Transmissão concluída.")
                    transmissao_concluida = True
                    break                

                elif (num_pacote_processado + 1) % WINDOW_SIZE == 0:
                    print(f"[SERVER] Fim da janela (Pacote {num_pacote_processado}). Enviando ACK cumulativo -> ACK_{ack_a_enviar}\n")
                    conexao_cliente.sendall(f"ACK_{ack_a_enviar}".encode('utf-8'))

                else:
                    print(f"[SERVER] Pacote {num_pacote_processado} no meio da janela. Aguardando fim da janela para enviar ACK...\n")
